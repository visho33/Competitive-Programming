...
void colorear(int v, int p, int c){
    nodosres[c].push_back(v);
    asignado[v] = true;
    for(auto u: newaristas[v]){
        if(u == p || asignado[u] == true) continue;
        colorear(u, v, c);
        aristasres[c].push_back({u, v});
    }
}
...
int main(){
    ...
    for(int gx = 0; gx<M; gx++){
        if(nodosres[gx].size() != 0) continue;
        int inicio = 0;
        for(int i = 0; i<N; i++){
            if(asignado[i] == false){
                inicio = i;
        ...
        while(q.size() > 0){
            ...
            for(auto u: newaristas[nodo]){
                if(int(nodosres[gx].size()) == G[gx]) break;
                if(bfs[u] == -1){
                    q.push(u);
                    bfs[u] = bfs[nodo];
                    if(asignado[u] == false){
                        bfs[u] = u;
                        nodosres[gx].push_back(u);
                        aristasres[gx].push_back({bfs[nodo], u});
                        asignado[u] = true;
                    }
                }
            }
    ...
    //cerr<<fixed<<setprecision(0)<<score0<<" "<<score1<<" "<<score2<<endl;
    //cerr<<fixed<<setprecision(3)<<cpu_time()<<endl;
