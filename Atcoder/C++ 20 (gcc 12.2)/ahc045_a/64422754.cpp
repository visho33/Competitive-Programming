...
void mejorar(int i){
    ...
    set<int> visitados;
    visitados.insert(i);
    random_shuffle(aristas[grupo].begin(), aristas[grupo].end());
...
int main(){
    ...
    for(int i = 0; i<N; i++){
        orden2[i].second = i;
        for(int j = 0; j<N; j++){
            orden2[i].first += dis(i, j);
        }
    }

    sort(orden.begin(), orden.end(), greater<ii>());
    sort(orden2.begin(), orden2.end());
    ...
    for(auto ggg: orden){
        ...
        int inicio = -1;
        for(int i = 0; i<N; i++){
            if(listo[orden2[i].second] == false){
                inicio = orden2[i].second;
                break;
            }
        }
        nodos[gx].push_back(inicio);
    ...
    for(int i = 0; i<M; i++){
        if(G[i] <= L){
            ...
            for(auto u: nodos[i]){
                mejorable.push_back(u);
            }
        }
    }
    
    while(Q > 0 && int(mejorable.size()) > 0){
        int i = rng()%int(mejorable.size());
        mejorar(mejorable[i]);
        Q--;
    }
